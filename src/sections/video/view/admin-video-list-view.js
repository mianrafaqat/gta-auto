...
export default function AdminVideoListView() {
  ...
  const canReset = !!(filters.name || filters.type || filters.status || filters.user);
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.video.admin.view(id));
    },
    [router]
  );
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.video.admin.edit(id));
    },
    [router]
  );
...
function applyFilter({ inputData, comparator, filters }) {
  const { name, type, status, user } = filters;
  ...
  if (name) {
    inputData = inputData.filter(
      (video) => video.title.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (type !== 'all') {
    inputData = inputData.filter((video) => video.type === type);
  }

  if (status !== 'all') {
    inputData = inputData.filter((video) => video.status === status);
  }

  if (user !== 'all') {
    inputData = inputData.filter((video) => video.userId === user);
  }
