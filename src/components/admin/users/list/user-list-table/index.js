...
import { AdminService, CarsService, UserService } from "src/services";
...
export default function UserListView() {
  ...
  const handleBanOrPermistUser = async (userData) => {
    try {
      const data = {
        userID: userData?._id,
        ban: !userData?.ban,
      };
      const res = await AdminService.banOrPermitUser(data);
      if (res?.data) {
        enqueueSnackbar(res?.data);
        fetchAllUsers();
      }
    } catch (err) {
      console.log("🚀 ~ handleBanOrPermistUser ~ err:", err);
                ...
                {dataFiltered
                  ...
                  .map((row) => (
                    <UserTableRow
                      ...
                      handleBanOrPermistUser={() => handleBanOrPermistUser(row)}
                    />
                  ))}
