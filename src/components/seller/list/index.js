...
const TABLE_HEAD = [
  ...
  { id: 'ebayUsername', label: 'eBay-Username' },
  { id: 'ebayEmail', label: 'eBay-Email' },
  { id: 'Email', label: 'Email' },
  { id: 'isAuthorised', label: 'Authorised' },
  { id: 'invitationDate', label: 'Invitation Date' },
...
export default function SellerListPage() {
  ...
  const handleGetSeller = async () => {
    const response = await SellerService.getSeller(table.page, table.rowsPerPage);
  ...
  const handleEditSeller = (row) => {
    router.push(paths.dashboard.seller.edit + `?eBayUserName=${row?.userId}&sellerId=${row?.id}`);
  };
  return (
    <>
        ...
        <Card>
          ...
          {/* <Grid container sx={{ p: 1 }} spacing={1}>
            {Object.entries(defaultFilters).map(([key, value]) => {
              // Convert camelCase to readable label format
              const label =
                key === 'id'
                  ? 'ID'
                  : key
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <SellerRows
                        ...
                        handleEditSeller={() => handleEditSeller(row)}
                      />
                    ))}
...
function applyFilter({ inputData, comparator, filterValues }) {
  ...
  let stabilizedThis = inputData?.length ? inputData?.map((el, index) => [el, index]) : [];
  ...
  if (ebayUsername) {
    inputData = inputData.filter(
      (user) => user.ebayUsername.toLowerCase().indexOf(ebayUsername.toLowerCase()) !== -1
    );
  }

  if (invitationEmail) {
    inputData = inputData.filter(
      (user) => user.invitationEmail.toLowerCase().indexOf(invitationEmail.toLowerCase()) !== -1
    );
  }

  if (ebayEmail) {
    inputData = inputData.filter(
      (user) => user.ebayEmail.toLowerCase().indexOf(ebayEmail.toLowerCase()) !== -1
    );
  }
